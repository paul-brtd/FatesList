...
from core import is_staff, log, MiniContext, BotAdminOp, request, MenuState, BotListView, BotState
...
import io
import uuid
from config import support_channel, general_support_channel, staff_ping_role, reports_channel, staff_apps_channel, ddr_channel, certify_channel
...
class TicketMenu(discord.ui.View):
    def __init__(self, bot, public):
        ...
        self.select_menu = _TicketCallback(bot=bot, placeholder="How can we help?", options=[])
        self.select_menu.add_option(label="General Support", value="support", description="General support on Fates List", emoji="🎫")
...
class _TicketCallback(discord.ui.Select):
    ...
    async def certify(self, interaction):
        res = await request("GET", MiniContext(interaction.user, self.bot), f"/api/users/{interaction.user.id}", staff = False)
        if res[0] == 404:
            return await interaction.response.send_message("You have not even logged in even once on Fates List!", ephemeral=True)
        profile = res[1]
        if not profile["approved_bots"]:
            return await interaction.response.send_message("You do not have any approved bots...", ephemeral=True)
        
        view = BotListView(self.bot, interaction, profile["approved_bots"], None, _CertifySelect)
        return await interaction.response.send_message("Please choose the bot you wish to request certification for", view=view, ephemeral=True)
...
class _ReportView(discord.ui.View):
    def __init__(self, interaction, bot):
        ...
        self.bot = bot
        self.select_menu = _ReportCallback(bot=bot, placeholder="What would you like to report?", options=[])
...
class _ReportCallback(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        def id_check(m):
            return m.content.isdigit() and m.author.id == interaction.user.id and isinstance(m.channel, discord.DMChannel) and len(m.content) in (16, 17, 18, 19, 20)
            
        try:  
            report_id = await self.bot.wait_for('message', check=id_check, timeout=180)
        except Exception as exc:
            return await interaction.followup.send("You took too long to respond!", ephemeral=True)
            
        id = int(report_id.content)
        
        res = await request("GET", MiniContext(interaction.user, self.bot), f"/api/users/{id}/obj", staff = False)
        if res[0] != 200:
            return await report_id.channel.send(f"Either this user does not exist or our API is down (got status code {res[0]})")
        
        if res[1]["bot"] and self.values[0] == "user":
            return await report_id.channel.send(f"This user is not a user but is a bot")
        
        if not res[1]["bot"] and self.values[0] == "bot":
            return await report_id.channel.send(f"This user is not a bot but is a regular user")
        
        def msg_check(m):
            return m.author.id == interaction.user.id and isinstance(m.channel, discord.DMChannel)
        
        await report_id.channel.send(f"Please tell us what this {self.values[0]} has done! Do not send an atttachment or picture, just in words!\n\nProvide message links if you have them!")
        
        try:  
            report_txt = await self.bot.wait_for('message', check=msg_check, timeout=180)
        except Exception as exc:
            return await interaction.followup.send("You took too long to respond!", ephemeral=True)
        
        view = _ReportProof(interaction, self.bot, self.values[0], report_id.channel, id, report_txt.content)
        return await report_id.channel.send("Do you have any attachments/extra proof etc. that you are willing to provide?", view=view)
...
class _ReportProof(discord.ui.View):
    ...
    async def _send_report(self, interaction, proof):
        ...
        await staff_channel.send(
            f"<@&{staff_ping_role}>",
            file=discord.File(io.BytesIO( "\n\n\n".join(report).encode() ), filename=f"report-{self.report_id}.txt"),
            allowed_mentions=AllowedMentions.all()
        )
        return await interaction.followup.send("Report successfully sent. Please note that spamming reports may lead to getting banned from Fates List!")
    ...
    async def proof_recv(self, button, interaction):
        ...
        await interaction.response.send_message("Please post any attachments/screenshots/extra info you are willing to share!")
        
        try:  
            extra = await self.bot.wait_for('message', check=msg_check, timeout=180)
        except Exception as exc:
            return await interaction.followup.send("You took too long to respond!")
        
        extra_info = [extra.content] + [attachment.url for attachment in extra.attachments]
    ...
    async def no_proof(self, button, interaction):
        self.stop()
        await interaction.response.defer()
        return await self._send_report(interaction, ["No extra proof/info willing to be provided"])
...
class _StaffAgeView(discord.ui.View):
    def __init__(self, interaction, bot):
        ...
        self.interaction = interaction
        self.bot = bot
        self.select_menu = _SelectAgeCallback(bot=bot, placeholder="Select your age range", options=[])
        self.select_menu.add_option(label="<=14", value="not_eligible", description="Less than or equal to 14 years old") # Not eligible
        self.select_menu.add_option(label="15-18", value="14-18", description="15 through 18 years old")
        self.select_menu.add_option(label="18+", value="adult (18+)", description="18+ years old")
        self.add_item(self.select_menu)
...
class _SelectAgeCallback(discord.ui.Select):
    def __init__(self, bot, *args, **kwargs):
        ...
        self.questions = {
            "tz": "Please DM me your timezone (the 3 letter code) to start your staff application",
            "exp": "Do you have experience being a bot reviewer? If so, from where and how long/much experience do you have? How confident are you at handling bots?",
            "lang": "How well do you know English? What other languages do you know? How good are you at speaking/talking/listening?",
            "why": "Why are you interested in being staff here at Fates List?",
            "contrib": "What do you think you can contribute to Fates List?",
            "talent": "What, in your opinion, are your strengths and weaknesses?",
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.values[0] == "not_eligible":
            return await interaction.followup.send("You are unfortunately not eligible to apply for staff!", ephemeral=True) 
        ...
        try:  
            tz = await self.bot.wait_for('message', check=tz_check, timeout=180)
        except Exception as exc:
            return await interaction.followup.send("You took too long to respond!", ephemeral=True)
        ...
        for q in self.questions.keys():
            ...
            await tz.channel.send(f"**{self.questions[q]}**\n\nUse at least 30 characters!")
        
            try:  
                ans = await self.bot.wait_for('message', check=app_ext_check, timeout=180)
            except Exception as exc:
                return await interaction.followup.send("You took too long to respond!", ephemeral=True)
        
            self.answers[q] = ans.content
        
        data = [f"Username: {interaction.user}\nUser ID: {interaction.user.id}\nAge Range: {self.values[0]}\nApplication ID: {self.app_id}"]
        for q in self.questions.keys():
        ...
        await staff_channel.send(
            f"<@&{staff_ping_role}>",
            file=discord.File(io.BytesIO( "\n\n\n".join(data).encode() ), filename=f"staffapp-{self.app_id}.txt"),
            allowed_mentions=AllowedMentions.all()
        )
...
class _DDRView(discord.ui.View):
    ...
    async def send_ddr(self, button, interaction):
        ...
        await interaction.followup.send("Please wait while we send your data deletion request...", ephemeral=True)
        ...
        await interaction.followup.send(content="Sent! Your data will be deleted within 24 hours!", ephemeral=True)
...
class _CertifySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if int(self.values[0]) == -1:    
            await interaction.followup.send("Please DM me the bot id you wish to certify", ephemeral=True)
            
            def id_check(m):
                return m.content.isdigit() and m.author.id == interaction.user.id and isinstance(m.channel, discord.DMChannel) and len(m.content) in (16, 17, 18, 19, 20)
            
            try:  
                id = await self.bot.wait_for('message', check=id_check, timeout=180)
                await id.channel.send(f"Ok, now go back to <#{interaction.channel_id}> to continue certification :)")
            except Exception as exc:
                return await interaction.followup.send("You took too long to respond!", ephemeral=True)
        ...
        res = await request("GET", MiniContext(interaction.user, self.bot), f"/api/bots/{id}?compact=false", staff = False)
        if res[0] != 200:
            return await interaction.followup.send(
        ...
        await channel.send(f"<@&{staff_ping_role}>", embed=embed, allowed_mentions=AllowedMentions.all())  
...
class Tickets(commands.Cog):
    ...
    async def _ticket(self, channel):   
        ...
        embed = Embed(title="Fates List Support", description="Hey there 👋! Thank you for contacting Fates List Support. How can we help you?")
