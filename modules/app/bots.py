...
async def bot_desc(request: Request, bot_id: int):
    bot = await db.fetchrow("SELECT css, html_long_description AS html_ld, long_description FROM bots WHERE bot_id = $1",int(bot_id))
    if bot:
        if not bot["html_ld"]:
        ...
        print(desc)
        return templates.TemplateResponse("bot.html", {"request":request, "long_description": True, "bot": {"long_description": desc,  "css": bot["css"]}})
    else:
