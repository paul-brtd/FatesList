...
async def bot_index(request: Request, bot_id: int):
    bot = await db.fetchrow("SELECT description, banner, certified, votes, servers, bot_id, invite FROM bots WHERE bot_id = $1 ORDER BY votes", bot_id)
    bot_info = await get_bot(bot["bot_id"])
    if bot_info:
        bot_obj = {"bot": bot, "avatar": bot_info["avatar"], "username": bot_info["username"], "votes": await human_format(bot["votes"]), "servers": await human_format(bot["servers"]), "description": bot["description"]}
    else:
        return abort(404)
    # TAGS
    tags_fixed = {}
