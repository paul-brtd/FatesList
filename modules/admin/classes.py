...
class BotListAdmin():
    ...
    def __init__(self, db, client, bot_id, mod, force = False):
        ...
        self.client = client
        self.channel = self.client.get_channel(bot_logs) # Bot log channel cached so we don't need to ask Discord
    ...
    async def _get_main_owner(self):
        ...
        return await self.db.fetchval("SELECT owner FROM bot_owner WHERE bot_id = $1 AND main = true", self.bot_id) # Return main owner from database

    async def _get_owners(self):
        return await self.db.fetch("SELECT owner FROM bot_owner WHERE bot_id = $1", self.bot_id)
    ...
    async def claim_bot(self):
        await self.db.execute("UPDATE bots SET state = $1 WHERE bot_id = $2", enums.BotState.under_review, self.bot_id) # Set it to under review in database
    ...
    async def unclaim_bot(self):
        await self.db.execute("UPDATE bots SET state = $1 WHERE bot_id = $2", enums.BotState.pending, self.bot_id) # Set it to pending in database
    ...
    async def approve_bot(self, feedback):
        await self.db.execute("UPDATE bots SET state = $1, verifier = $2 WHERE bot_id = $3", enums.BotState.approved, self.mod, self.bot_id)
    ...
    async def unverify_bot(self, reason):
        await self.db.execute("UPDATE bots SET state = $1 WHERE bot_id = $2", enums.BotState.pending, self.bot_id)
    ...
    async def deny_bot(self, reason):
        owner = await self._get_main_owner()
        await self.db.execute("UPDATE bots SET state = $1, verifier = $2 WHERE bot_id = $3", enums.BotState.denied, self.mod, self.bot_id)
    ...
    async def ban_bot(self, reason):
        ...
        try:
            ...
            pass
        await self.db.execute("UPDATE bots SET state = 4 WHERE bot_id = $1", self.bot_id)
    ...
    async def requeue_bot(self, reason):
        ...
        await self.db.execute("UPDATE bots SET state = 1 WHERE bot_id = $1", self.bot_id)
    ...
    async def unban_bot(self, reason):
        ...
        embed.add_field(name="Reason", value = reason)
        await self.channel.send(embed = embed)
        await self.db.execute("UPDATE bots SET state = 0 WHERE bot_id = $1", self.bot_id)
        await bot_add_event(self.bot_id, enums.APIEvents.bot_unban, {"user": self.str_mod, "reason": reason})

    async def certify_bot(self):
        await self.db.execute("UPDATE bots SET state = 6 WHERE bot_id = $1", self.bot_id)
    ...
    async def transfer_bot(self, reason, new_owner):
        owner = await self._get_main_owner()
        await self.db.execute("UPDATE bot_owner SET owner = $1 WHERE bot_id = $2 AND main = true", new_owner, self.bot_id) 
    ...
    async def delete_bot(self, reason):
        lock = await self.db.fetchval("SELECT lock FROM bots WHERE bot_id = $1", self.bot_id)
    ...
    async def root_update(self, reason, old_state, new_state):
        await self.db.execute("UPDATE bots SET state = $1 WHERE bot_id = $2", new_state, self.bot_id)
    ...
    async def reset_votes(self, reason):
        ...
        if self.bot_id == 0: # Recursive, reset all votes
            bots = await self.db.fetch("SELECT bot_id, votes FROM bots")
            epoch = time.time()
            for bot in bots:
                await self.db.execute("INSERT INTO bot_stats_votes_pm (bot_id, epoch, votes) VALUES ($1, $2, $3)", bot["bot_id"], epoch, bot["votes"])
            await self.db.execute("UPDATE bots SET votes = 0")
            await self.db.execute("UPDATE users SET vote_epoch = NULL")
            ...
            await self.db.execute("UPDATE bots set votes = 0 WHERE bot_id = $1", self.bot_id)
    ...
    async def lock_bot(self, lock, reason = "This bot was locked by staff, its owner or with its owners permission to protect against malicious or accidental editing!"):
        await self.db.execute("UPDATE bots SET lock = $1 WHERE bot_id = $2", lock, self.bot_id)
    ...
    async def unlock_bot(self, reason = "This bot was unlocked by staff, its owner or with its owners permission to make changes to the bot!"):
        await self.db.execute("UPDATE bots SET lock = $1 WHERE bot_id = $2", enums.BotLock.unlocked, self.bot_id)
