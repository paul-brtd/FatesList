...
async def get_widget(
    ...
    if isinstance(bgcolor, str):
        ...
        bgcolor = (
            floor(int(bgcolor)) if bgcolor.isdigit(
            ) or bgcolor.isdecimal() else bgcolor
        )
    if isinstance(textcolor, str):
        textcolor = textcolor.split(".")[0]
        textcolor = (
            floor(int(textcolor))
            if textcolor.isdigit() or textcolor.isdecimal()
            else textcolor
        )
    ...
    bt.add_task(
        ...
        {
            "m": {"e": event},
            "ctx": {"user": request.session.get("user_id"), "widget": True},
        },
        type=_type,
    )
    if target_type == enums.ReviewType.bot:
        data = {
            "bot": bot,
            "user": await get_bot(
                target_id, worker_session=request.app.state.worker_session
            ),
        }
    else:
        data = {
    ...
    if format == enums.WidgetFormat.json:
        ...
        return await templates.TemplateResponse(
            "widget.html", {"request": request} | data
        )
        ...
        try:
            widget_img.paste(
                Image.alpha_composite(avatar_pil_bg, votes_pil), (120, 115)
            )
        except:
        ...
        try:
            widget_img.paste(
                Image.alpha_composite(avatar_pil_bg, server_pil), (120, 95)
            )
        except:
        ...
        d.text(
            ...
            font=ImageFont.truetype(
                font, 10, layout_engine=ImageFont.LAYOUT_RAQM),
        )

        # Bot name
        d.text(
            (the_area(d.textsize(str(bot_obj["username"]))[
             0], widget_img.size[0]), 5),
            str(bot_obj["username"]),
            fill=textcolor,
            font=ImageFont.truetype(
                font, 16, layout_engine=ImageFont.LAYOUT_RAQM),
        )
