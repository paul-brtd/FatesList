...
@router.patch(
    ...
    dependencies=[
        Depends(Ratelimiter(global_limit=Limit(times=7, minutes=3))),
...
@router.get(
    ...
    dependencies=[
        Depends(
            Ratelimiter(global_limit=Limit(times=7, seconds=5),
                        operation_bucket="random_guild"))
    ],
    operation_id="fetch_random_server",
)
async def fetch_random_server(request: Request,
                              guild_id: int,
                              lang: str = "default"):
    """
    ...
    """
    if guild_id != 0:
        return api_error("This guild cannot use the fetch random guild API")
    ...
    bot["guild_id"] = str(
        bot["guild_id"]
    )  # Make sure bot id is a string to prevent corruption issues in javascript
    bot["formatted"] = {
        "votes": human_format(bot["votes"]),
        "guild_count": human_format(bot["guild_count"]),
    }
    bot["description"] = cleaner.clean_html(intl_text(
        bot["description"], lang))  # Prevent XSS attacks in short description
...
@router.get(
    ...
    dependencies=[
        Depends(
            Ratelimiter(global_limit=Limit(times=5, minutes=1),
...
async def fetch_server(
    ...
    if sensitive:
        await server_auth_check(guild_id,
                                request.headers.get("Authorization", ""))
        sensitive_dat = await db.fetchrow(
            "SELECT invite_channel, user_whitelist, user_blacklist FROM servers WHERE guild_id = $1",
            guild_id,
        )
        api_ret |= dict(sensitive_dat)
        api_ret["invite_channel"] = (str(api_ret["invite_channel"])
                                     if api_ret["invite_channel"] else None)
    ...
    api_ret["tags"] = [
        dict((await db.fetchrow(
            "SELECT name, id, iconify_data FROM server_tags WHERE id = $1",
            id))) for id in api_ret["_tags"]
    ]

    api_ret["user"] = dict((await db.fetchrow(
        "SELECT guild_id AS id, name_cached AS username, '0000' AS disc, avatar_cached AS avatar FROM servers WHERE guild_id = $1",
        guild_id,
    )))
    ...
    if not sensitive:
        await redis_db.set(f"guildcache-{guild_id}",
                           orjson.dumps(api_ret),
                           ex=60 * 60 * 8)
...
async def server_exists(request: Request, guild_id: int):
    count = await db.fetchval(
        "SELECT guild_id FROM servers WHERE guild_id = $1", guild_id)
    return PlainTextResponse("", status_code=200 if count else 404)


@router.get("/{guild_id}/widget",
            operation_id="get_server_widget",
            deprecated=True)
async def server_widget(
    request: Request,
    bt: BackgroundTasks,
    guild_id: int,
    ...
    return RedirectResponse(
        f"/api/widgets/{guild_id}?target_type={enums.ReviewType.server}&format={format.name}&textcolor={textcolor}&bgcolor={bgcolor}"
    )


@router.get(
    "/{guild_id}/ws_events",
    dependencies=[Depends(server_auth_check)],
