...
async def tags(request: Request, tag: str):
    ...
    fetch = await db.fetch(f"SELECT DISTINCT bots.bot_id, bots.description, bots.state, bots.banner, bots.votes, bots.guild_count, bots.invite FROM bots INNER JOIN bot_tags ON bot_tags.bot_id = bots.bot_id WHERE bot_tags.tag = $1 AND (bots.state = 0 OR bots.state = 6) ORDER BY bots.votes DESC LIMIT 15", tag)
