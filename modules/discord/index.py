from config import partners, privacy_policy
...
async def exp1(request: Request):
    ...
    if data["logged_in"]:
        data["user"] = await get_user(
            data["user_id"], worker_session=request.app.state.worker_session
        )

    return PrettyJSONResponse(
        {"message": "Please send a screenshot of this page and send it", "data": data}
    )
...
@router.get("/{vanity}")
async def vanity_bot_uri(
    request: Request, bt: BackgroundTasks, vanity: str, redirect: bool = False
):
    return await vanity_redirector(
        request, vanity, render_bot, {"bt": bt, "api": False}, guild_page
    )
...
async def features_view(request: Request, name: str):
    if name not in features.keys():
        return abort(404)
    bots = await db.fetch(
        "SELECT description, banner_card AS banner, votes, guild_count, bot_id, invite, state FROM bots, unnest(features) feature WHERE feature = $1 and (state = 0 or state = 6) ORDER BY votes DESC LIMIT 12",
        name,
    )
    ...
    return await templates.TemplateResponse(
        ...
        {"request": request, "name": name,
            "feature": features[name], "bots": bot_obj},
    )
...
async def stats_page(request: Request, full: bool = False):
    ...
    certified = await do_index_query(
        state=[enums.BotState.certified], limit=None, worker_session=worker_session
    )
    bot_amount = await db.fetchval(
        "SELECT COUNT(1) FROM bots WHERE state = 0 OR state = 6"
    )
    queue = await do_index_query(
        state=[enums.BotState.pending],
        limit=None,
        add_query="ORDER BY created_at ASC",
        worker_session=worker_session,
    )
    under_review = await do_index_query(
        state=[enums.BotState.under_review],
        limit=None,
        add_query="ORDER BY created_at ASC",
        worker_session=worker_session,
    )
    if full:
        denied = await do_index_query(
            state=[enums.BotState.denied],
            limit=None,
            add_query="ORDER BY created_at ASC",
            worker_session=worker_session,
        )
        banned = await do_index_query(
            state=[enums.BotState.banned],
            limit=None,
            add_query="ORDER BY created_at ASC",
            worker_session=worker_session,
        )
    data = {
        ...
        "denied_amt": await db.fetchval(
            "SELECT COUNT(1) FROM bots WHERE state = $1", enums.BotState.denied
        ),
        "banned": banned if full else [],
        "banned_amt": await db.fetchval(
            "SELECT COUNT(1) FROM bots WHERE state = $1", enums.BotState.banned
        ),
    ...
    return await templates.TemplateResponse(
        "admin.html", {"request": request} | data
...
async def login_get(
    ...
    pretty: Optional[str] = "to access this page",
):
...
async def tos_page(request: Request):
    return await templates.TemplateResponse(
        "tos.html", {"request": request, "policy": privacy_policy}
    )
...
async def rules_page(request: Request):
    return await templates.TemplateResponse(
        "rules.html", {"request": request, "policy": rules}
    )
...
async def fates_partners(request: Request):
    return await templates.TemplateResponse(
        "partners.html", {"request": request, "partners": partners}
    )
