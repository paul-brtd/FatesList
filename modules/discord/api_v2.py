...
async def get_bot_reviews(request: Request, bot_id: int, page: int):
    reviews = await parse_reviews(bot_id, page = page)
    if reviews[0] == []:
        return abort(404)
    return {"reviews": reviews[0], "average_stars": reviews[1], "pager": {"total_count": reviews[2], "total_pages": reviews[3], "per_page": reviews[4], "from": ((page - 1) * reviews[4]) + 1, "to": (page - 1) * reviews[4] + reviews[3]}}
