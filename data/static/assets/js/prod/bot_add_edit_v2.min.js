...
function testHook(a,b){headers={Authorization:context.bot_token};$j.ajax({url:"/api/bots/"+context.bot_id+"/votes/test?user_id="+context.user_id,dataType:"json",headers:headers,type:"POST",processData:!1,contentType:"application/json"});modalShow("Sent Test Query","If you do not get the test webhook, make sure you have editted the bot with the webhook url first by adding the Webhook URL and THEN clicking Edit and then try again")};
